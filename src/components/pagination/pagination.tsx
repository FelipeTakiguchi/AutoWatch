...
export default function Pagination() {
  ...
  function renderPageNumbers() {
    ...
    const last =
      initial + sibling_count * 2 < totalPages
        ? initial + sibling_count * 2
        : totalPages;

    for (let i = initial; i <= last; i++) {
      if (i == page) {
        list.push(
          <button key={i} className={`selected_link nav_button`}>
            {i}
          </button>
        );
      } else {
    ...
    <>
      {totalPages > 0 && (
        <div className="navigation_bar">
          ...
          >
            <Image className="arrow" src={backIcon} alt="Back Arrow" />
          </button>
          ...
          <div className="paginator">{renderPageNumbers()}</div>
          ...
          <button
            ...
            className="arrow_button"
          >
            <Image className="arrow" src={forwardIcon} alt="Foward Arrow" />
          </button>
        </div>
      )}
    </>
